...
class Fact(OperableCE, Bindable, dict, metaclass=Validable):
        ...
        for name, field in self.__fields__.items():
            if name in self:
                try:
                    ...
                    raise ValueError(
                        "Invalid value on field %r for fact %r" % (name, self)
                    )
            elif field.mandatory:
                raise ValueError(
                    "Mandatory field %r is not defined for fact %r" % (name, self)
                )
            else:
                pass
    ...
    def as_dict(self):
        ...
        return {k: unfreeze(v) for k, v in self.items() if not self.is_special(k)}
