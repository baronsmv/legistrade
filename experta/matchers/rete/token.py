...
class TokenInfo(namedtuple("_TokenInfo", ["data", "context"])):
    ...
    def __new__(cls, data, context):
        ...
        return super(TokenInfo, cls).__new__(
            cls, frozenset(data), frozenset(context.items())
        )
...
class Token(namedtuple("_Token", ["tag", "data", "context"])):
    ...
    def __new__(cls, tag, data, context=None):
        ...
        try:
            assert isinstance(tag, cls.TagType), "tag must be of `Token.TagType` type"
            assert isinstance(data, Fact) or all(
                isinstance(f, Fact) for f in data
