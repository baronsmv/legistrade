...
CheckFunction = namedtuple("CheckFunction", ["key_a", "key_b", "expected", "check"])
...
class TypeCheck(Check, namedtuple("_TypeCheck", ["fact_type"])):
    ...
    def __call__(self, fact):
        ...
        log("type(%s) == %s = %r", fact, self.fact_type.__name__, res)
...
class FeatureCheck(
    Check, namedtuple("_FeatureCheck", ["what", "how", "check", "expected"])
):
    _instances = dict()
    ...
    def __new__(cls, what, how):
        ...
        if key not in cls._instances:
            cls._instances[key] = super(Check, cls).__new__(
                cls, what, how, check_function.check, check_function.expected
            )
    ...
    def __call__(self, data, is_fact=True):
        if is_fact:
            if isinstance(self.what, str):
                record = data
                if not self.what.startswith("__") and not self.what.endswith("__"):
                    try:
    ...
    def _(pce, what=None):
        ...
        return CheckFunction(
            key_a=L, key_b=(pce.value, pce.__bind__), expected=pce, check=equal_literal
        )
    ...
    def _(pce, what=None):
        ...
        key_b = ("ID", id(pce.match), pce.__bind__)

        return CheckFunction(key_a=P, key_b=key_b, expected=pce, check=match_predicate)
    ...
    def _(pce, what=None):
        ...
        return CheckFunction(
            key_a=W, key_b=pce.__bind__, expected=pce, check=wildcard_match
        )
    ...
    def _(pce, what=None):
        def not_equal(actual, expected):
            ...
            if isinstance(subresult, Mapping):
                newresult = {(False, k): v for k, v in subresult.items()}
                return newresult
            else:
        ...
        return CheckFunction(key_a=NOTFC, key_b=key_b, expected=key_b, check=not_equal)
    ...
    def _(pce, what=None):
        ...
        return CheckFunction(key_a=ANDFC, key_b=key_b, expected=key_b, check=and_match)
    ...
    def _(pce, what=None):
        ...
        return CheckFunction(key_a=ORFC, key_b=key_b, expected=key_b, check=or_match)
