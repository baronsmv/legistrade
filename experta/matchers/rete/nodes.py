...
class BusNode(mixins.AnyChild, mixins.NoMemory, Node):
    """
    The Bus Node.
...
class WhereNode(mixins.AnyChild, mixins.HasMatcher, mixins.NoMemory, OneInputNode):
    """
...
class FeatureTesterNode(
    mixins.AnyChild, mixins.HasMatcher, mixins.NoMemory, OneInputNode
):
    """
    Feature Tester Node.
            ...
            if isinstance(match, Mapping):
                for key, value in match.items():
                    if isinstance(key, tuple):  # Negated condition
                        if key[1] in token.context and token.context[key[1]] == value:
                            return False
                    else:
                        if token.context.get(key, value) != value:
                            return False
                        if (False, key) in token.context and token.context[
                            (False, key)
                        ] == value:
                            return False
...
class OrdinaryMatchNode(mixins.AnyChild, mixins.HasMatcher, TwoInputNode):
    """
    ...
    def __activation(self, token, branch_memory, matching_memory, is_left=True):
        """
        ...
        for other_data, other_context in matching_memory:
            ...
            if match:
                ...
                newcontext = {
                    k: v for k, v in token.context.items() if isinstance(k, str)
                }
                ...
                newtoken = Token(token.tag, token.data | other_data, newcontext)
    ...
    def _activate_left(self, token):
        ...
        self.__activation(token, self.left_memory, self.right_memory, is_left=True)

    def _activate_right(self, token):
        """Node right activation."""
        self.__activation(token, self.right_memory, self.left_memory, is_left=False)


class ConflictSetNode(mixins.AnyChild, OneInputNode):
    """
...
class NotNode(mixins.AnyChild, mixins.HasMatcher, TwoInputNode):
    """
    Not Node.
    ...
    def _activate_right(self, token):
        ...
        for left in self.left_memory:
            if self.matcher(dict(left.context), token.context):
                ...
                if (newcount == 0 and inc == -1) or (newcount == 1 and inc == 1):
                    if inc == -1:
