        ...
        if len(elem) == 1:
            ...
            for f, s in zip(elem, elem[1:]):
                if isinstance(s, TEST):
                    ...
                    left_branch.add_child(current_node, current_node.activate_left)
                    right_branch.add_child(current_node, current_node.activate_right)
            return current_node

    @_wire_rule.register(OR)
    def _(elem):
        raise SyntaxError("You can't use an OR clause inside FORALL or EXISTS")
