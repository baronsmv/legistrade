...
class Rule(ConditionalElement):
    ...
    def __hash__(self):
        return hash(tuple(self) + (self._wrapped, self._wrapped_self, self.salience))

    def __eq__(self, other):
        if isinstance(other, self.__class__):
            self_data = tuple(self) + (self._wrapped, self._wrapped_self, self.salience)
            other_data = tuple(other) + (
                other._wrapped,
                other._wrapped_self,
                other.salience,
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._wrapped is None:
            ...
            if self._wrapped_args:
                kwargs = {k: v for k, v in kwargs.items() if k in self._wrapped_args}
