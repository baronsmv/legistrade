...
class KnowledgeEngine:
    ...
    def __init__(self):
        ...
        if isinstance(self.__matcher__, type) and issubclass(
            self.__matcher__, abstract.Matcher
        ):
            self.matcher = self.__matcher__(self)
        else:
            raise TypeError("__matcher__ must be a subclass of Matcher")

        if isinstance(self.__strategy__, type) and issubclass(
            self.__strategy__, abstract.Strategy
        ):
    ...
    def __declare(self, *facts):
        ...
        if any(f.has_field_constraints() for f in facts):
            raise TypeError("Declared facts cannot contain conditional elements")
